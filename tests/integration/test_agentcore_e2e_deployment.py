...
def test_agent_runtime_arn_format(stack_outputs):
    ...
    assert re.match(pattern, runtime_arn), f"AgentRuntimeArn format invalid: {runtime_arn}"
...
def test_agent_runtime_endpoint_https(stack_outputs):
    ...
    assert re.match(pattern, endpoint_url), f"EndpointUrl format invalid: {endpoint_url}"
...
def test_execution_role_has_bedrock_policy(stack_outputs, iam_client):
    ...
    try:
        ...
        has_bedrock_permission = any(
            "bedrock" in arn.lower() for arn in policy_arns
        )
...
def test_resource_tagging(stack_outputs, cloudformation_client):
    try:
        ...
        tags_response = cloudformation_client.describe_stacks(StackName="AgentCoreStack")
        stack_tags = {tag["Key"]: tag["Value"] for tag in tags_response["Stacks"][0].get("Tags", [])}
