def test_alb_dns_contract(network_stack_outputs):
    ...
    assert (
        "AlbDnsName" in network_stack_outputs
    ), "AlbDnsName output not found in NetworkStack"

    alb_dns = network_stack_outputs["AlbDnsName"]
    # ALB DNS format: name-id.region.elb.amazonaws.com
    assert (
        ".elb.amazonaws.com" in alb_dns
    ), f"ALB DNS {alb_dns} does not match expected format"
...
def test_alb_properties(network_stack_outputs, elbv2_client):
    ...
    ), "Load balancer must be Application Load Balancer"
