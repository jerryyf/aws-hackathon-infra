...
def test_agentcore_execution_role_policy_contract_bedrock(agentcore_stack_outputs, iam_client, load_contract_json):
    ...
    assert has_bedrock_perms, "Execution role must have Bedrock model invocation permissions from contract"


def test_agentcore_execution_role_policy_contract_ecr(agentcore_stack_outputs, iam_client, load_contract_json):
...
def test_agentcore_execution_role_policy_contract_cloudwatch(agentcore_stack_outputs, iam_client, load_contract_json):
    ...
    assert has_logs_perms, "Execution role must have CloudWatch Logs permissions from contract"


def test_agentcore_execution_role_policy_contract_secrets(agentcore_stack_outputs, iam_client, load_contract_json):
    ...
    assert has_secrets_perms, "Execution role must have Secrets Manager permissions from contract"


def test_agentcore_execution_role_policy_contract_kms(agentcore_stack_outputs, iam_client, load_contract_json):
...
def test_agentcore_execution_role_trust_policy(agentcore_stack_outputs, iam_client):
    ...
    principals = [stmt["Principal"]["Service"] for stmt in trust_policy["Statement"] 
                  if "Service" in stmt["Principal"]]
    assert "bedrock.amazonaws.com" in principals, "Trust policy must allow bedrock.amazonaws.com"


def test_agentcore_execution_role_trust_policy_contract(agentcore_stack_outputs, iam_client, load_contract_json):
    ...
    assert expected_service in actual_principals or "bedrock.amazonaws.com" in actual_principals, \
        f"Trust policy must allow {expected_service} (or bedrock.amazonaws.com)"
    
    # Verify AssumeRole action
    for stmt in actual_trust_policy["Statement"]:
        if "sts:AssumeRole" in stmt.get("Action", []) or stmt.get("Action") == "sts:AssumeRole":
...
def test_agentcore_execution_role_bedrock_permissions(agentcore_stack_outputs, iam_client):
    ...
    assert len(managed_policies) + len(inline_policies) > 0, "Execution role must have policies attached"
...
def test_agentcore_execution_role_ecr_permissions(agentcore_stack_outputs, iam_client):
    ...
    for policy_name in inline_response["PolicyNames"]:
        ...
        for stmt in policy_doc.get("Statement", []):
            ...
            ecr_actions = ["ecr:GetAuthorizationToken", "ecr:BatchCheckLayerAvailability", 
...
def test_agentcore_execution_role_cloudwatch_permissions(agentcore_stack_outputs, iam_client):
...
def test_agentcore_execution_role_secrets_manager_permissions(agentcore_stack_outputs, iam_client):
...
def test_agentcore_execution_role_output(agentcore_stack_outputs):
    ...
    assert "ExecutionRoleArn" in agentcore_stack_outputs, "ExecutionRoleArn output missing"
    
    role_arn = agentcore_stack_outputs["ExecutionRoleArn"]
    assert role_arn.startswith("arn:aws:iam::"), "ExecutionRoleArn must be valid IAM role ARN"
