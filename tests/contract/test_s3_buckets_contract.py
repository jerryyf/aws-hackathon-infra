def test_s3_buckets_contract(storage_stack_outputs):
    ...
    for output_name in required_outputs:
        assert output_name in storage_stack_outputs, f"{output_name} output not found in StorageStack"
...
def test_s3_bucket_properties(storage_stack_outputs, s3_client):
    ...
    for bucket_name in bucket_names:
        ...
        assert response["ResponseMetadata"]["HTTPStatusCode"] == 200, f"Bucket {bucket_name} not found"
        
        # Verify versioning enabled
        versioning = s3_client.get_bucket_versioning(Bucket=bucket_name)
        assert versioning.get("Status") == "Enabled", f"Bucket {bucket_name} must have versioning enabled"
        ...
        assert config["BlockPublicAcls"] is True, f"Bucket {bucket_name} must block public ACLs"
        assert config["BlockPublicPolicy"] is True, f"Bucket {bucket_name} must block public policy"
        assert config["IgnorePublicAcls"] is True, f"Bucket {bucket_name} must ignore public ACLs"
        assert config["RestrictPublicBuckets"] is True, f"Bucket {bucket_name} must restrict public buckets"
