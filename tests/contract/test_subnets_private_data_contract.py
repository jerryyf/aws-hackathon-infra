...
def test_private_data_subnets_contract(network_stack_outputs):
    ...
    assert "PrivateDataSubnetIds" in network_stack_outputs, "PrivateDataSubnetIds output not found in NetworkStack"
    
    subnet_ids = network_stack_outputs["PrivateDataSubnetIds"].split(",")
    assert len(subnet_ids) == 2, f"Expected 2 private data subnet IDs, got {len(subnet_ids)}"
...
def test_private_data_subnets_properties(network_stack_outputs, ec2_client):
    ...
    assert len(response["Subnets"]) == 2, f"Expected 2 private data subnets, found {len(response['Subnets'])}"
