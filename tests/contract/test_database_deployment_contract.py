def test_database_deployment_readiness(database_stack_outputs):
    ...
    for output_name in required_outputs:
        assert output_name in database_stack_outputs, f"{output_name} output not found in DatabaseStack"


def test_database_deployment_outputs(database_stack_outputs, rds_client, opensearch_client):
    ...
    if ".proxy-" in rds_endpoint:
        ...
        try:
            ...
            assert len(rds_response["DBProxies"]) == 1, f"RDS proxy {proxy_name} not accessible"
            assert rds_response["DBProxies"][0]["Status"] == "available", \
        ...
        assert len(rds_response["DBClusters"]) == 1, f"RDS cluster {cluster_id} not accessible"
        assert rds_response["DBClusters"][0]["Status"] == "available", \
    ...
    for domain in domains:
        ...
        if domain_info["DomainStatus"]["Endpoint"] == os_endpoint or \
           domain_info["DomainStatus"].get("Endpoints", {}).get("vpc") == os_endpoint:
            domain_found = True
            assert domain_info["DomainStatus"]["Processing"] is False, \
