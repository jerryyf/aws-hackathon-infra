...
def test_private_app_subnets_contract(network_stack_outputs):
    ...
    assert "PrivateAppSubnetIds" in network_stack_outputs, "PrivateAppSubnetIds output not found in NetworkStack"
    
    subnet_ids = network_stack_outputs["PrivateAppSubnetIds"].split(",")
    
    # Validate exactly 2 subnet IDs per contract
    assert len(subnet_ids) == 2, f"Expected 2 private app subnet IDs, got {len(subnet_ids)}"
...
def test_private_app_subnets_properties(network_stack_outputs, ec2_client):
    ...
    assert len(response["Subnets"]) == 2, f"Expected 2 private app subnets, found {len(response['Subnets'])}"
