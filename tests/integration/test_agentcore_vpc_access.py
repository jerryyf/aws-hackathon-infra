...
def test_bedrock_agentcore_gateway_endpoint_exists(ec2_client, network_stack_outputs):
    ...
    assert endpoint["VpcEndpointType"] == "Interface"
    assert (
        endpoint["ServiceName"] == "com.amazonaws.us-east-1.bedrock-agentcore.gateway"
    )
    assert endpoint["VpcId"] == vpc_id
    assert endpoint["State"] == "available"
    assert endpoint["PrivateDnsEnabled"] is True
...
def test_agentcore_endpoints_use_private_agent_subnets(
    ec2_client, network_stack_outputs
):
    endpoint_id = network_stack_outputs["BedrockAgentCoreEndpointId"]
    ...
    for endpoint in response["VpcEndpoints"]:
        ...
        for subnet_id in endpoint_subnets:
            assert (
                subnet_id in private_agent_subnet_ids
            ), f"Endpoint subnet {subnet_id} not in PrivateAgent subnets"
...
def test_agentcore_endpoints_span_multiple_azs(ec2_client, network_stack_outputs):
    ...
    assert (
        len(subnet_ids) >= 2
    ), "AgentCore endpoint should span at least 2 subnets for HA"
...
def test_agentcore_runtime_security_group_exists(ec2_client, network_stack_outputs):
    ...
    assert (
        sg["GroupDescription"] == "Security group for Bedrock AgentCore agent runtimes"
    )


def test_agentcore_runtime_security_group_egress_rules(
    ec2_client, network_stack_outputs
):
    sg_id = network_stack_outputs["AgentCoreRuntimeSecurityGroupId"]
    ...
    assert (
        https_rule_found
    ), "Security group should allow HTTPS (443) egress to 0.0.0.0/0"


def test_agentcore_runtime_security_group_no_default_egress(
    ec2_client, network_stack_outputs
):
    sg_id = network_stack_outputs["AgentCoreRuntimeSecurityGroupId"]
    ...
    assert (
        not all_traffic_rule_found
    ), "Security group should not have default allow-all egress rule"
...
def test_private_agent_subnets_exist(ec2_client, network_stack_outputs):
    ...
    assert (
        len(private_agent_subnet_ids) >= 2
    ), "Should have at least 2 private agent subnets"
    ...
    for subnet in response["Subnets"]:
        ...
        ), "Private agent subnets should not auto-assign public IPs"
...
def test_private_agent_subnets_span_multiple_azs(ec2_client, network_stack_outputs):
    ...
    assert (
        len(azs) >= 2
    ), "Private agent subnets should span multiple availability zones"


def test_private_agent_subnets_have_nat_gateway_routes(
    ec2_client, network_stack_outputs
):
    private_agent_subnet_ids = network_stack_outputs["PrivateAgentSubnetIds"].split(",")
    ...
    for subnet in response["Subnets"]:
        ...
        assert (
            len(route_table_response["RouteTables"]) > 0
        ), f"Subnet {subnet['SubnetId']} should have associated route table"
        ...
        assert (
            nat_gateway_route_found
        ), f"Private agent subnet {subnet['SubnetId']} should have NAT gateway route"
...
def test_vpc_endpoint_security_groups(ec2_client, network_stack_outputs):
    ...
    assert (
        len(endpoint.get("Groups", [])) > 0
    ), "VPC endpoint should have security groups attached"
