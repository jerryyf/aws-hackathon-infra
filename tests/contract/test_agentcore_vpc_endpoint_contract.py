...
def test_bedrock_agentcore_endpoint_policy_contract(
    stack_outputs, ec2_client, load_contract_json
):
    ...
    if "PolicyDocument" in endpoint and endpoint["PolicyDocument"]:
        ...
        runtime_stmt = next(
            (
                s
                for s in contract["Statement"]
                if s["Sid"] == "AllowAgentRuntimeInvocation"
            ),
            None,
        )
        assert (
            runtime_stmt
        ), "Contract must define AllowAgentRuntimeInvocation statement"
...
def test_bedrock_agentcore_gateway_endpoint_output(stack_outputs):
    ...
    if "BedrockAgentCoreGatewayEndpointId" not in outputs:
        pytest.skip(
            "BedrockAgentCoreGatewayEndpointId not found - AgentCore not deployed"
        )
...
def test_agentcore_runtime_security_group_output(stack_outputs):
    ...
    if "AgentCoreRuntimeSecurityGroupId" not in outputs:
        pytest.skip(
            "AgentCoreRuntimeSecurityGroupId not found - AgentCore not deployed"
        )
...
def test_agentcore_runtime_security_group_properties(stack_outputs, ec2_client):
    ...
    assert (
        sg["GroupName"].startswith("NetworkStack-AgentCoreRuntimeSecurityGroup")
        or "AgentCoreRuntime" in sg["GroupName"]
    )
